...
function defaultConfig(options: Required<Pick<InitOptions, "source" | "agent" | "label" | "maxConcurrent">> & Pick<InitOptions, "model">, projectName: string, branch: string): RelayConfig {
  ...
  return relayConfigSchema.parse({
    ...
    agents: { [options.agent]: { command: agentCommand, args: agentArgs, environment: {}, models: [], defaultModelProfile: "selected", modelArgument: "--model", ...(options.agent === "claude" ? { reasoningEffortArgument: "--effort" } : {}), promptDelivery: { mode: "argument" } } },
