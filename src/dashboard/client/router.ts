export type DashboardPage = "home" | "repositories" | "workflows" | "executions" | "workers" | "plugins";
...
const pages = new Set<DashboardPage>(["home", "repositories", "workflows", "executions", "workers", "plugins"]);
...
export function parseDashboardRoute(pathname: string, search = ""): DashboardRoute {
  ...
  if (segments.some((segment) => segment === undefined)) return { page: "home" };
  ...
  if (values[0] === "projects" && values[1]) {
    ...
    if (page === "workflows") return values.length === 4 && values[3] ? { page, projectId, workflowId: values[3] } : values.length === 3 ? { page, projectId } : { page: "home", projectId };
    if (page === "executions") return values.length === 4 && values[3] ? { page, projectId, executionId: values[3] } : values.length === 3 ? { page, projectId } : { page: "home", projectId };
  ...
  if (page === "workflows") return values.length === 2 && values[1]
    ? { page, workflowId: values[1] }
    : values.length === 1 ? legacyProjectRoute(page, search) : { page: "home" };
  if (page === "executions") return values.length === 2 && values[1]
...
function legacyProjectRoute(page: DashboardPage, search: string): DashboardRoute {
...
export function dashboardRoutePath(route: DashboardRoute): string {
  const prefix = route.projectId ? `/projects/${encodeURIComponent(route.projectId)}` : "";
  const page = route.page === "home" ? "" : `/${route.page}`;
  const detail = route.page === "workflows" && route.workflowId
    ? `/${encodeURIComponent(route.workflowId)}`
    : route.page === "executions" && route.executionId
      ? `/${encodeURIComponent(route.executionId)}`
      : "";
  return `${prefix}${page}${detail}` || "/";
}

export function routeForPage(page: DashboardPage, projectId?: string, detail?: string): DashboardRoute {
  return {
    page,
    ...(projectId ? { projectId } : {}),
